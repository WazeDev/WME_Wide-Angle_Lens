...
namespace WMEWAL_Streets {
    ...
    const updateText = '<ul>' +
        '<li>Find segments with missing city on primary, alt, either, or both</li>' +
        '<li>Find segments with house numbers and no city</li>' +
...
//                || settings.NonNeutralRoutingPreference
            ...
            if (savedSegments.indexOf(s.getID()) === -1 ) {
                ...
                if (sid != null && !newSegment) {
                    thisStreet = extentStreets.find(function (e) {
                        ...
                        if (includeLockLevel) {
                            matches &&= (e.lockLevel === (s.attributes.lockRank | 0) + 1);
                        }
                        if (includeLastEditor) {
                            matches &&= e.lastEditor === lastEditor.userName;
                        }
                        if (includeCreatedEditor) {
                            matches &&= e.createdEditor === createdEditor.userName;
                        }
    ...
    async function Init(): Promise<void> {
        ...
        let allOK = true;
        const objectToCheck: Array<string> = [
            "W.app",
            "W.model.states",
            "OpenLayers",
