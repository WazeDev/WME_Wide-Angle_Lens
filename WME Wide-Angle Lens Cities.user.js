...
(function (WMEWAL_Cities) {
    ...
    function GetTab() {
        ...
        html += "<tr><td style='padding-left: 20px'>" +
            ...
            "<div><input type='checkbox' id='_wmewalCitiesRoadTypeAlley' name='_wmewalCitiesRoadType' value='" + WMEWAL.RoadType.Alley.toString() + "'/>" +
            "<label for='_wmewalCitiesRoadTypeAlley' style='margin-left: 8px'>" + WMEWAL.TranslateRoadType(WMEWAL.RoadTypeBitmaskToWazeRoadType(WMEWAL.RoadType.Alley)) + "</label></div>" +
    ...
    function updateLayers() {
        ...
        selectLayer.empty();
        var layers = [];
        for (var ixLayer = 0; ixLayer < W.map.layers.length; ixLayer++) {
            var layer = W.map.layers[ixLayer];
    ...
    function updateUI() {
        ...
        $("#_wmewalCitiesRoadTypeAlley").prop("checked", settings.RoadTypeMask & WMEWAL.RoadType.Alley);
    ...
    function validateSettings() {
        ...
        if (layerUniqueName !== null && layerUniqueName.length > 0) {
            var layers = W.map.getLayersBy("uniqueName", layerUniqueName);
            if (layers.length === 0) {
                ...
                if ((WMEWAL.WazeRoadTypeToRoadTypeBitmask(segment.attributes.roadType) & settings.RoadTypeMask) &&
                    ...
                    if (!segment.isInRoundabout()) {
                        ...
                        for (var rIx = 0; rIx < r.segIDs.length; rIx++) {
                            addSegment(W.model.segments.get(r.segIDs[rIx]), incorrectCity, cityShouldBe, r.id);
                        }
                    }
                }
            }
